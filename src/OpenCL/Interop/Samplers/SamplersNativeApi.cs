...
namespace OpenCL.NET.Interop.Samplers
{
    ...
    public static class SamplersNativeApi
    {
        ...
        ///     The built-in functions to read from an image in a kernel take a sampler as an argument. The sampler arguments to
        ///     the image read function can be sampler objects created using OpenCL functions and passed as argument values to the
        ///     kernel or can be samplers declared inside a kernel.
        /// </summary>
        ...
        /// </param>
        /// <returns>
        ///     Returns a valid non-zero sampler object and <see cref="errorCode" /> is set to <c>Result.Success</c> if the sampler
        ...
        ///     <c>Result.InvalidValue</c> if the property name in <see cref="samplerProperties" /> is not a supported property
        ///     name, if the value specified for a supported property name is not valid, or if the same property name is specified
        ///     more
        ///     than once.
        ///     <c>Result.InvalidOperation</c> if <see cref="images" /> are not supported by any device associated with
        ///     <see cref="context" />.
        ...
        ///     Specifies a list of sampler property names and their corresponding values. Each sampler property name is
        ///     immediately followed by the corresponding desired value. The list is terminated with 0. If a supported property and
        ///     its value
        ///     is not specified in <see cref="samplerProperties" />, its default value will be used.
        ///     <see cref="samplerProperties" /> can be <c>null</c> in which case the default values for supported sampler
        ...
        ///     Specifies the size in bytes of memory pointed to by <see cref="parameterValue" />. This
        ///     size must be greater or equal to the size of return type as described in the table above.
        /// </param>
        ...
        ///     <see cref="parameterValueSize" /> is less the size of return type as described in the table above and
