...
namespace OpenCL.NET.Interop.Events
{
    ...
    public static class EventsNativeApi
    {
        ...
        ///     Specifies the event object being queried./param>
        ...
        ///         <c>Result.InvalidValue</c> if <see cref="parameterName" /> is not one of the supported values or if size in
        ///         bytes specified by <see cref="parameterValueSize" /> is less than size of return type and
        ///         <see cref="parameterValue" /> is
        ///         not a <c>null</c> value or if information to query given in <see cref="parameterName" /> cannot be queried for
        ///         event.
        ///         <c>Result.OutOfResources</c> if there is a failure to allocate resources required by the OpenCL implementation
        ///         on the device.
        ...
        ///     specific command identified by this event has completed (or terminated) and there are no commands in the
        ///     command-queues of
        ///     a context that require a wait for this event to complete. Using this function to release a reference that was not
        ///     obtained by creating the object or by calling <see cref="RetainEvent" /> causes undefined behavior. Developers
        ///     should
        ///     be careful when releasing their last reference count on events created by <see cref="CreateUserEvent" /> that have
        ///     not yet been set to status of <c>CommandExecutionStatus.Complete</c> or an error. If the user event was used in the
        ///     <see cref="eventWaitList" /> argument passed to a Enqueue*** API or another application host thread is waiting for
        ///     it in <see cref="WaitForEvents" />, those commands and host threads will continue to wait for the event status to
        ///     reach
        ///     <c>CommandExecutionStatus.Complete</c> or error, even after the user has released the object. Since in this
        ///     scenario the developer has released his last reference count to the user event, it would be in principle no longer
        ///     valid for
        ///     him to change the status of the event to unblock all the other machinery. As a result the waiting tasks will wait
        ///     forever, and associated events, memory objects, command queues and contexts are likely to leak. In-order command
        ///     queues
        ...
        ///     Specifies the command execution status for which the callback is registered. The command execution callback values
        ///     for which a callback can be registered are <c>CommandExecutionStatus.Submitted</c>,
        ///     <c>CommandExecutionStatus.Running</c>, or <c>CommandExecutionStatus.Complete</c>. There is no guarantee that the
        ///     callback functions registered for various execution status values for an event will be called in the exact order
        ///     that
        ///     the execution status of a command changes. Furthermore, it should be noted that receiving a call back for an event
        ///     with a status other than <c>CommandExecutionStatus.Complete</c>, in no way implies that the memory model or
        ///     execution
        ///     model as defined by the OpenCL specification has changed. For example, it is not valid to assume that a
        ///     corresponding memory transfer has completed unless the event is in a state <c>CommandExecutionStatus.Complete</c>.
        ///     The callback
        ///     function registered for a <see cref="commandExecutionCallbackType" /> value of
        ///     <c>CommandExecutionStatus.Complete</c> will be called when the command has completed successfully or is abnormally
        ///     terminated.
        /// </param>
        /// <param name="notificationCallback">
        ///     The event callback function that can be registered by the application. This callback function may be called
        ...
        ///     <see cref="eventCommandExecutionStatus" /> represents the execution status of command for which this callback
        ///     function is invoked. If the callback is called as the result of the command associated with event being abnormally
        ///     terminated, an appropriate error code for
        ...
        /// </param>
        /// <param name="userData">
        ///     Will be passed as the <see cref="userData" /> argument when <see cref="notificationCallback" />
        ///     is called. <see cref="userData" /> can be <c>null</c>.
        /// </param>
        /// <returns>
        ///     Returns <c>Result.Success</c> if the function is executed successfully. Otherwise, it returns the following:
        ///     <c>Result.InvalidEvent</c> if <see cref="event" /> is not a valid event object.
        ///     <c>Result.InvalidValue</c> if <see cref="notificationCallback" /> is <c>null</c> or if
        ///     <see cref="commandExecutionCallbackType" /> is not <c>CommandExecutionStatus.Complete</c>.
        ///     <c>Result.OutOfResources</c> if there is a failure to allocate resources required by the OpenCL implementation on
        ///     the device.
        ///     <c>Result.OutOfHostMemory</c> if there is a failure to allocate resources required by the OpenCL implementation on
        ///     the host.
