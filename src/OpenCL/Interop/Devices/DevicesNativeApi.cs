...
namespace OpenCL.NET.Interop.Devices
{
    ...
    public static class DevicesNativeApi
    {
        ...
        /// </summary>
        /// <param name="platform">
        ///     Refers to the platform ID returned by <see cref="GetPlatformIds"/< or can be <c>null</c>. If
        ///     <see cref="platform" /> is <c>null</c>, the behavior is implementation-defined.
        /// </param>
        /// <param name="deviceType">
        ///     A bitfield that identifies the type of OpenCL device. The <see cref="deviceType" /> can be
        ///     used to query specific OpenCL devices or all OpenCL devices available.
        /// </param>
        /// <param name="numberOfEntries">
        ///     The number of device entries that can be added to <see cref="devices" />. If
        ///     <see cref="devices" /> is not <c>null</c>, the <see cref="numberOfEntries" /> must be greater than zero.
        /// </param>
        /// <param name="devices">
        ///     A list of OpenCL devices found. The device values returned in <see cref="devices" /> can be used to identify a
        ///     specific OpenCL device. If <see cref="devices" /> argument is <c>null</c>, this argument is ignored. The number of
        ///     OpenCL devices returned is the mininum of the value specified by <see cref="numberOfEntries" /> or the number of
        ///     OpenCL devices whose type matches <see cref="deviceType" />.
        /// </param>
        /// <param name="numberOfDevicesReturned">
        ///     The number of OpenCL devices available that match <see cref="deviceType". If
        ///     <see cref="numberOfDevicesReturned" /> is <c>null</c>, this argument is ignored.
        /// </param>
        /// <returns>
        ...
        ///     <c>Result.InvalidValue</c> if <see cref="numberOfEntries" /> is equal to zero and <see cref="devices" /> is not
        ///     <c>null</c> or if both <see cref="numberOfDevicesReturned" /> and <see cref="devices" /> are <c>null</c>.
        ...
        /// </summary>
        /// <param name="device">
        ///     A device returned by <see cref="GetDeviceIds" />. May be a device returned by <see cref="GetDeviceIds" /> or a
        ///     sub-device created by <see cref="CreateSubDevices" />. If device is a sub-device, the specific information for the
        ...
        ///     Specifies the size in bytes of memory pointed to by <see cref="parameterValue" />.
        ///     This size in bytes must be greater than or equal to the size of return type specified.
        /// </param>
        /// <param name="parameterValue">
        ///     A pointer to memory location where appropriate values for a given
        ///     <see cref="parameterName" />. If <see cref="parameterValue" /> is <c>null</c>, it is ignored.
        /// </param>
        ...
        ///     is
        ///     not a <c>null</c> value or if <see cref="parameterName" /> is a value that is available as an extension and the
        ...
        ///     Specifies how <see cref="inDevice" /> is to be partitioned, described by a partition name and its corresponding
        ///     value. Each partition name is immediately followed by the corresponding desired value. The list is terminated with
        ///     0.
        /// </param>
        /// <param name="numberOfDevices">
        ///     Size of memory pointed to by <see cref="outDevices" /> specified as the number of device
        ///     entries.
        /// </param>
        /// <param name="outDevices"></param>
        /// <param name="numberOfDevicesReturned">
        ///     The buffer where the OpenCL sub-devices will be returned. If <see cref="outDevices" /> is <c>null</c>, this
        ///     argument is ignored. If <see cref="outDevices" /> is not <c>null</c>, <see cref="numberOfDevices" /> must be
        ///     greater than
        ///     or equal to the number of sub-devices that device may be partitioned into according to the partitioning scheme
        ///     specified in properties.
        /// </param>
        /// <returns>
        ///     Returns <c>Result.Success</c> if the function is executed successfully. Otherwise, it returns the following:
        ///     <c>Result.InvalidDevice</c> if <see cref="device" /> is not valid.
        ///     <c>Result.InvalidValue</c> if values specified in <see cref="properties" /> are not valid or if values specified in
        ///     <see cref="properties" /> are valid but not supported by the device. If <see cref="outDevices" /> is not
        ///     <c>null</c> and
        ///     <see cref="numberOfDevices" /> is less than the number of sub-devices created by the partition scheme.
        ///     <c>Result.DevicePartitionFailed</c> if the partition name is supported by the implementation but
        ///     <see cref="inDevice" /> could not be further partitioned.
        ///     <c>Result.InvalidDevicePartitionCount</c> if the partition name specified in properties is
        ///     <c>DevicePartitionProperty.PartitionByCounts</c> and the number of sub-devices requested exceeds the maximum number
        ///     of sub-devices or the total
        ///     number of compute units requested exceeds the maximum number of compute units for <see cref="inDevice" />, or the
        ///     number of compute units requested for one or more sub-devices is less than zero or the number of sub-devices
        ///     requested
        ...
        ///     <see cref="CreateSubDevices" />. If device is a root level device i.e. a device returned by
        ///     <see cref="GetDeviceIDs" />, the device
        ...
        ///     replace a default device command queue created with <see cref="CreateCommandQueueWithProperties" /> and the
        ...
        ///     The command queue that is to be replace the current default command queue as default command
        ...
        ///     <c>Result.InvalidDevice</c> if <see cref="device" /> is not a valid device object.
        ///     <c>Result.InvalidCommandQueue</c> if <see cref="commandQueue" /> is not a valid command queue for
        ///     <see cref="device" />.
        ///     <c>Result.OutOfResources</c> if there is a failure to allocate resources required by the OpenCL implementation on
        ///     the device.
        ///     <c>Result.OutOfHostMemory</c> if there is a failure to allocate resources required by the OpenCL implementation on
        ...
        ///     device profiling timer returned by <see cref="GetDeviceInformation" /> and the
        ...
        ///     which device timer was returned. The resolution of the timer may be queried via <see cref="GetPlatformInfo" />
        ///     and the flag <c>PlatformInformation.HostTimerResolution</c>.
        /// </param>
        /// <returns>
        ///     Returns <c>Result.Success</c> with a time value in <see cref="hostTimestamp" /> if provided. Otherwise, it returns
