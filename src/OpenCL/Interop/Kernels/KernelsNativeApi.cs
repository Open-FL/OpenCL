...
namespace OpenCL.NET.Interop.Kernels
{
    ...
    public static class KernelsNativeApi
    {
        ...
        ///     <c>Result.InvalidKernelName</c> if the function definition for __kernel function given by <see cref="kernelName" />
        ///     such as the number of arguments, the argument types are not the same for all devices for which the program
        ...
        /// <param name="numberOfKernels">
        ///     The size of memory pointed to by <see cref="kernels" /> specified as the number of kernel
        ///     entries.
        /// </param>
        /// <param name="kernels">
        ///     The buffer where the kernel objects for kernels in <see cref="program" /> will be returned. If
        ///     <see cref="kernels" /> is <c>null</c>, it is ignored. If <see cref="kernels" /> is not <c>null</c>,
        ///     <see cref="numberOfKernels" /> must be
        ///     greater than or equal to the number of kernels in program.
        /// </param>
        /// <param name="numberOfKernelsReturned">
        ///     The number of kernels in <see cref="program" />. If
        ///     <see cref="numberOfKernels" /> is <c>null</c>, it is ignored.
        /// </param>
        /// <returns>
        ///     Returns <c>Result.Success</c> if the kernel objects are successfully allocated. Otherwise, it returns one of the
        ///     following errors:
        ///     <c>Result.InvalidProgram</c> if <see cref="program" /> is not a valid program object.
        ///     <c>Result.InvalidProgramExecutable</c> if there is no successfully built executable for any device in
        ///     <see cref="program" />.
        ///     <c>Result.InvalidValue</c> if <see cref="kernels" /> is not <c>null</c> and <see cref="numberOfKernels" /> is less
        ...
        /// </param>
        /// <param name="parameterName">Specifies the argument information to query.</param>
        /// <param name="inputValueSize">
        ///     Specifies the size in bytes of memory pointed to by <see cref="inputValue" />. This size
        ///     must be equal to the size of the input type.
        /// </param>
        /// <param name="inputValue">
        ///     A pointer to memory where the appropriate parameterization of the query is passed from. If
        ///     <see cref="inputValue" /> is <c>null</c>, it is ignored.
        /// </param>
        /// <param name="parameterValueSize">
        ///     Used to specify the size in bytes of memory pointed to by
        ///     <see cref="parameterValue" />. This size must be greater or equal to the size of the return type.
        ...
        ///     <see cref="parameterValueSize" /> is less than size of return type and <see cref="parameterValue" /> is not a
        ///     <c>null</c> value or
        ///     if <see cref="parameterName" /> is <c>KernelSubGroupInformation.SubGroupCountForNDRange</c> and the size in bytes
        ///     specified by <see cref="inputValueSize" /> is not valid or if <see cref="inputValue" /> is <c>null</c>.
        ///     <c>Result.InvalidKernel</c> if <see cref="kernel" /> is not a valid kernel object.
        ///     <c>Result.OutOfResources</c> if there is a failure to allocate resources required by the OpenCL implementation on
        ///     the device.
        ///     <c>Result.OutOfHostMemory</c> if there is a failure to allocate resources required by the OpenCL implementation on
