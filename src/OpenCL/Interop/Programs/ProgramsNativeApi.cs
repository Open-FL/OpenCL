...
namespace OpenCL.NET.Interop.Programs
{
    ...
    public static class ProgramsNativeApi
    {
        ...
        /// <param name="strings">
        ///     An array of <see cref="count" /> pointers to optionally null-terminated character strings that
        ///     make up the source code.
        /// </param>
        /// <param name="lengths">
        ///     An array with the number of chars in each string (the string length). If an element in <see cref="lengths" /> is
        ///     zero, its accompanying string is null-terminated. If lengths is <c>null</c>, all strings in the strings argument
        ///     are
        ///     considered null-terminated. Any length value passed in that is greater than zero excludes the null terminator in
        ///     its count.
        /// </param>
        /// <param name="errorCode">Returns an appropriate error code. If errorCode is <c>null</c>, no error code is returned.</param>
        /// <returns>
        ///     Returns a valid non-zero program object and <see cref="errorCode" /> is set to <c>Result.Success</c> if the program
        ...
        ///     A pointer to a list of devices that are in <see cref="context" />. <see cref="deviceList" />
        ///     must be a non-<c>null</c> value. The binaries are loaded for devices specified in this list.
        /// </param>
        /// <param name="lengths">
        ...
        /// </param>
        /// <param name="binaries">
        ///     An array of pointers to program binaries to be loaded for devices specified by <see cref="deviceList" />. For each
        ///     device given by <c>deviceList[i]</c>, the pointer to the program binary for that device is given by
        ///     <c>binaries[i]</c>
        ///     and the length of this corresponding binary is given by <c>lengths[i]</c>. <c>lengths[i]</c> cannot be zero and
        ///     <c>binaries[i]</c> cannot be a <c>null</c> pointer.
        ...
        ///     The program binary can consist of either or both of device-specific code and/or implementation-specific
        ///     intermediate representation (IR) which will be converted to the device-specific code.
        /// </param>
        /// <param name="binaryStatus">
        ///     Returns whether the program binary for each device specified in <see cref="deviceList" /> was loaded successfully
        ///     or not. It is an array of <see cref="numberOfDevices" /> entries and returns <c>Result.Success</c> in
        ///     <c>binaryStatus[i]</c> if the binary was successfully loaded for the device specified by <c>deviceList[i]</c>.
        ///     Otherwise returns <c>Result.InvalidValue</c> if <c>lengths[i]</c> is zero or if <c>binaries[i]</c> is a <c>null</c>
        ///     value or <c>Result.InvalidBinary</c> in <c>binaryStatus[i]</c> if the program binary is not a valid binary for the
        ///     specified device. If <see cref="binaryStatus" /> is <c>null</c>, it is ignored.
        /// </param>
        ...
        ///     returned in <see cref="errorCode" />:
        ///     <c>Result.InvalidContext</c> if <see cref="context" /> is not a valid context.
        ///     <c>Result.InvalidValue</c> if <see cref="deviceList" /> is <c>null</c>, <see cref="numberOfDevices" /> is zero, or
        ///     if lengths or binaries are <c>null</c> or if any entry in <c>lengths[i]</c> or <c>binaries[i]</c> is <c>null</c>.
        ///     <c>Result.InvalidDevice</c> if OpenCL devices listed in <see cref="deviceList" /> are not in the list of devices
        ///     associated with <see cref="context" />.
        ///     <c>Result.InvalidBinary</c> if an invalid program binary was encountered for any device.
        ///     <see cref="binaryStatus" /> will return specific status for each device.
        ///     <c>Result.OutOfResources</c> if there is a failure to allocate resources required by the OpenCL implementation on
        ///     the device.
        ///     <c>Result.OutOfHostMemory</c> if there is a failure to allocate resources required by the OpenCL implementation on
        ///     the host.
        ...
        /// <param name="numberOfDevices">The number of devices listed in <see cref="deviceList" />.</param>
        /// <param name="deviceList">
        ///     A pointer to a list of devices that are in <see cref="context" />. <see cref="deviceList" /> must be a non-
        ///     <c>null</c> value. The built-in kernels are loaded for devices specified in this list. The devices associated with
        ///     the program
        ///     object will be the list of devices specified by <see cref="deviceList" />. The list of devices specified by
        ///     <see cref="deviceList" /> must be devices associated with <see cref="context" />.
        /// </param>
        ...
        ///     <c>Result.InvalidContext</c> if <see cref="context" /> is not a valid context.
        ///     <c>Result.InvalidValue</c> if <see cref="deviceList" /> is <c>null</c>, <see cref="numberOfDevices" /> is zero, or
        ///     if <see cref="kernelNames" /> is <c>null</c> or <see cref="kernelNames" /> contains a kernel name that is not
        ///     supported
        ...
        /// <param name="context">Must be a valid OpenCL context.</param>
        /// <param name="il">
        ///     A pointer to a length-byte block of memory containing SPIR-V or an implementation-defined intermediate
        ///     language.
        /// </param>
        ...
        ///     returned in <see cref="errorCode" />:
        ///     <c>Result.InvalidContext</c> if <see cref="context" /> is not a valid context.
        ///     <c>Result.InvalidValue</c> if <see cref="il" /> is <c>null</c>, <see cref="length" /> is zero, or if
        ///     <see cref="length" />-byte memory pointed to by <see cref="il" /> does not contain well-formed intermediate
        ///     language input that can be
        ...
        /// <param name="numberOfDevices">The number of devices listed in <see cref="deviceList" />.</param>
        /// <param name="deviceList">
        ///     A pointer to a list of devices associated with <see cref="program" />. If <see cref="deviceList" /> is a
        ///     <c>null</c> value, the program executable is built for all devices associated with <see cref="program" /> for which
        ///     a source
        ///     or binary has been loaded. If <see cref="deviceList" /> is a non-<c>null</c> value, the program executable is built
        ///     for devices specified in this list for which a source or binary has been loaded.
        /// </param>
        /// <param name="options">
        ///     A pointer to a null-terminated string of characters that describes the build options to be used
        ///     for building the program executable. Certain options are ignored when program is created with IL.
        /// </param>
        /// <param name="notificationCallback">
        ///     A function pointer to a notification routine. The notification routine is a callback function that an application
        ///     can register and which will be called when the program executable has been built (successfully or unsuccessfully).
        ///     If <see cref="notificationCallback" /> is not <c>null</c>, <see cref="BuildProgram" /> does not need to wait for
        ///     the build to complete and can return immediately once the build operation can begin. The build operation can begin
        ///     if
        ///     the context, program whose sources are being compiled and linked, list of devices and build options specified are
        ///     all valid and appropriate host and device resources needed to perform the build are available. If
        ///     <see cref="notificationCallback" /> is <c>null</c>, <see cref="BuildProgram" /> does not return until the build has
        ///     completed. This callback function may be called asynchronously by the OpenCL implementation. It is the
        ///     application’s responsibility to ensure that the callback function is thread-safe.
        /// </param>
        /// <param name="userData">
        ///     Passed as an argument when <see cref="notificationCallback" /> is called.
        ///     <see cref="userData" /> can be <c>null</c>.
        /// </param>
        ...
        ///     A pointer to a list of devices associated with <see cref="program" />. If <see cref="deviceList" /> is a
        ///     <c>null</c> value, the compile is performed for all devices associated with <see cref="program" />. If
        ///     <see cref="deviceList" />
        ...
        /// <param name="options">
        ///     A pointer to a null-terminated string of characters that describes the compilation options to be
        ///     used for building the program executable. Certain options are ignored when program is created with IL.
        /// </param>
        /// <param name="numberOfInputHeaders">
        ///     Specifies the number of programs that describe headers in the array referenced by
        ///     <see cref="inputHeaders" />.
        /// </param>
        ...
        ///     An array that has a one to one correspondence with <see cref="inputHeaders" />. Each entry in
        ///     <see cref="headerIncludeNames" /> specifies the include name used by source in program that comes from an embedded
        ///     header. The
        ///     corresponding entry in <see cref="inputHeaders" /> identifies the program object which contains the header source
        ///     to be used. The embedded headers are first searched before the headers in the list of directories specified by the
        ///     <c>–I</c> compile option. If multiple entries in <see cref="headerIncludeNames" /> refer to the same header name,
        ///     the first one encountered will be used.
        /// </param>
        /// <param name="notificationCallback">
        ///     A function pointer to a notification routine. The notification routine is a callback function that an application
        ///     can register and which will be called when the program executable has been built (successfully or unsuccessfully).
        ///     If <see cref="notificationCallback" /> is not <c>null</c>, <see cref="CompileProgram" /> does not need to wait for
        ///     the compiler to complete and can return immediately once the compilation can begin. The compilation can begin if
        ///     the
        ///     context, program whose sources are being compiled, list of devices, input headers, programs that describe input
        ///     headers and compiler options specified are all valid and appropriate host and device resources needed to perform
        ///     the
        ///     compile are available. If <see cref="notificationCallback" /> is <c>null</c>, <see cref="CompileProgram" /> does
        ///     not return until the compiler has completed. This callback function may be called asynchronously by the OpenCL
        ...
        ///     context and creates a library or executable.
        /// </summary>
        /// <param name="context">Must be a valid OpenCL context.</param>
        /// <param name="numberOfDevices">
        ///     A pointer to a list of devices associated with <see cref="program" />. If <see cref="deviceList" /> is a
        ...
        ///     executable. For each device in <see cref="deviceList" /> or if <see cref="deviceList" /> is <c>null</c> the list of
        ///     devices associated with context, the following cases occur:
        ///     - All programs specified by <see cref="inputPrograms" /> contain a compiled binary or library for the device. In
        ///     this case, a link is performed to generate a program executable for this device.
        ...
        ///     A function pointer to a notification routine. The notification routine is a callback function that an application
        ///     can register and which will be called when the program executable has been built (successfully or unsuccessfully).
        ///     If <see cref="notificationCallback" /> is not <c>null</c>, <see cref="LinkProgram" /> does not need to wait for the
        ///     linker to complete and can return immediately once the linking operation can begin. Once the linker has completed,
        ///     the <see cref="notificationCallback" /> callback function is called which returns the program object returned by
        ///     <see cref="LinkProgram" />. The application can query the link status and log for this program object. This
        ///     callback
        ///     function may be called asynchronously by the OpenCL implementation. It is the application’s responsibility to
        ///     ensure that the callback function is thread-safe.
        ///     If <see cref="notificationCallback" /> is <c>null</c>, <see cref="LinkProgram" /> does not return until the linker
        ///     has completed.
        /// </param>
        /// <param name="userData">
        ///     Passed as an argument when <see cref="notificationCallback" /> is called.
        ...
        ///     <c>Result.InvalidValue</c> if <see cref="parameterName" /> is not valid, or if size in bytes specified by
        ///     <see cref="parameterValueSize" /> is less than the size of return type and <see cref="parameterValue" /> is not
        ///     <c>null</c>.
        ///     <c>Result.InvalidProgram</c> if <see cref="program" /> is not a valid program object.
        ///     <c>Result.InvalidProgramExecutable</c> if <see cref="parameterName" /> is <c>ProgramInformation.NumberOfKernels</c>
        ///     or <c>ProgramInformation.KernelNames</c> and a successful program executable has not been built for at least one
        ...
        /// <param name="program">Specifies the program object being queried.</param>
        /// <param name="device">
        ///     Specifies the device for which build information is being queried. <see cref="device" /> must be a
        ///     valid device associated with <see cref="program" />.
        /// </param>
        /// <param name="parameterName">Specifies the information to query.</param>
        /// <param name="parameterValueSize">
        ///     Used to specify the size in bytes of memory pointed to by
