...
namespace OpenCL.NET.Interop.Memory
{
    ...
    public static class MemoryNativeApi
    {
        ...
        /// <param name="flags">
        ///     An enumeration that is used to specify allocation and usage information such as the memory arena that should be
        ///     used to allocate the buffer object and how it will be used. If value specified for <see cref="flags" /> is 0, the
        ...
        ///     A pointer to the buffer data that may already be allocated by the application. The size of
        ///     the buffer that <see cref="hostPointer" /> points to must be greater or equal than size bytes.
        /// </param>
        ...
        /// </param>
        /// <returns>
        ///     Returns a valid non-zero buffer object and <see cref="errorCode" /> is set to <c>Result.Success</c> if the buffer
        ///     object is created successfully. Otherwise, it returns a <c>null</c> value and an error value in
        ///     <see cref="errorCode" />.
        /// </returns>
        ...
        /// <param name="flags">
        ///     An enumeration that is used to specify allocation and usage information about the sub-buffer memory object being
        ///     created and is described in the table below. If the <c>MemoryFlag.ReadWrite</c>, <c>MemoryFlag.ReadOnly</c> or
        ///     <c>MemoryFlag.WriteOnly</c> values are not specified in <see cref="flags" />, they are inherited from the
        ///     corresponding memory access qualifers associated with <see cref="buffer" />. The <c>MemoryFlag.UseHostPointer</c>,
        ///     <c>MemoryFlag.AllocateHostPointer</c> and <c>MemoryFlag.CopyHostPointer</c> values cannot be specified in
        ///     <see cref="flags" /> but are inherited from the corresponding memory access qualifiers associated with
        ///     <see cref="buffer" />.
        ///     If <c>MemoryFlag.CopyHostPointer</c> is specified in the memory access qualifier values associated with
        ///     <see cref="buffer" /> it does not imply any additional copies when the sub-buffer is created from
        ///     <see cref="buffer" />. If
        ///     the <c>MemoryFlag.HostWriteOnly</c>, <c>MemoryFlag.HostReadOnly</c> or <c>MemoryFlag.HostNoAccess</c> values are
        ///     not specified in <see cref="flags" />, they are inherited from the corresponding memory access qualifiers
        ///     associated
        ...
        /// <param name="flags">
        ///     An enumeration that is used to specify allocation and usage information about the image memory
        ///     object being created.
        /// </param>
        /// <param name="imageFormat">A pointer to a structure that describes format properties of the image to be allocated.</param>
        /// <param name="imageDescription">
        ///     A pointer to a structure that describes type and dimensions of the image to be
        ///     allocated.
        /// </param>
        ...
        /// </param>
        /// <returns>
        ///     Returns a valid non-zero image object and <see cref="errorCode" /> is set to <c>Result.Success</c> if the image
        ///     object is created successfully. Otherwise, it returns a <c>null</c> value with an error value returned in
        ///     <see cref="errorCode" />.
        /// </returns>
        ...
        /// <param name="flags">
        ///     An enumeration that is used to specify allocation and usage information such as the memory arena that should be
        ///     used to allocate the pipe object and how it will be used. Only <c>MemoryFlag.ReadOnly</c>,
        ///     <c>MemoryFlag.WriteOnly</c>,
        ///     <c>MemoryFlag.ReadWrite</c>, and <c>MemoryFlag.HostNoAccess</c> can be specified when creating a pipe object. If
        ///     value specified for <see cref="flags" /> is 0, the default is used which is
        ///     <c>MemoryFlag.ReadWrite | MemoryFlag.HostNoAccess</c>.
        /// </param>
        /// <param name="pipePacketSize">Size in bytes of a pipe packet.</param>
        /// <param name="pipeMaximumNumberOfPackets">
        ///     Specifies the pipe capacity by specifying the maximum number of packets the
        ///     pipe can hold.
        /// </param>
        /// <param name="properties">
        ///     A list of properties for the pipe and their corresponding values. Each property name is immediately followed by the
        ///     corresponding desired value. The list is terminated with 0. In OpenCL 2.0, properties must be <c>null</c>.
        /// </param>
        /// <param name="errorCode">
        ///     Returns an appropriate error code. If <see cref="errorCode" /> is <c>null</c>, no error code is
        ///     returned.
        /// </param>
        /// <returns>
        ///     Returns a valid non-zero pipe object and <see cref="errorCode" /> is set to <c>Result.Success</c> if the pipe
        ///     object is created successfully. Otherwise, it returns a <c>null</c> value with an error value returned in
        ///     <see cref="errorCode" />.
        /// </returns>
        ...
        ///     <see cref="ImageFormat" /> structure supported by the OpenCL implementation. If image_formats is <c>null</c>, it is
        ///     ignored.
        /// </param>
        /// <param name="numberOfImageFormats">
        ///     The actual number of supported image formats for a specific <see cref="context" />
        ///     and values specified by <see cref="flags" />. If <see cref="numberOfImageFormats" /> is <c>null</c>, it is ignored.
        /// </param>
        /// <returns>
        ...
        ///     <c>Result.InvalidValue</c> if <see cref="flags" /> or <see cref="imageType" /> are not valid or if
        ///     <see cref="numberOfEntries" /> is 0 and <see cref="imageFormats" /> is not <c>null</c>.
        ///     <c>Result.OutOfResources</c> if there is a failure to allocate resources required by the OpenCL implementation on
        ///     the device.
        ///     <c>Result.OutOfHostMemory</c> if there is a failure to allocate resources required by the OpenCL implementation on
        ///     the host.
        ...
        /// <param name="parameterValueSize">
        ///     Used to specify the size in bytes of memory pointed to by
        ///     <see cref="parameterValue" />. This size must be greater or equla to the size of the return type.
        /// </param>
        /// <param name="parameterValue">
        ///     A pointer to memory where the appropriate result being queried is returned. If
        ///     <see cref="parameterValue" /> is <c>null</c>, it is ignored.
        ...
        ///     <see cref="parameterValue" />. If <see cref="parameterValueSizeReturned" /> is <c>null</c>, it is ignored.
        /// </param>
        /// <returns>
        ///     Returns <c>Result.Success</c> if the function is executed successfully. Otherwise, it returns the following:
        ...
        ///     <see cref="parameterValueSize" /> is less than the size of return type as described in the table above and
        ...
        ///     memoryObject: The memory object being deleted. When the user callback is called by the implementation, this memory
        ///     object is no longer valid. The memory object is only provided for reference purposes.
        ///     userData: A pointer to user supplied data.
        /// </param>
        /// <param name="userData">
        ///     Will be passed as the userData argument when <see param="notificationCallback" /> is called.
        ...
        ///     The scan-line pitch in bytes. This must be 0 if <see cref="hostPointer" /> is <c>null</c> and can be either 0 or
        ///     greater than or equal to <see cref="imageWidth" /> * size of element in bytes if <see cref="hostPointer" /> is not
        ///     <c>null</c>.
        ///     If <see cref="hostPointer" /> is not <c>null</c> and <see cref="imageRowPitch" /> is equal to 0,
        ///     <see cref="imageRowPitch" /> is calculated as <see cref="imageWidth" /> * size of element in bytes. If
        ///     <see cref="imageRowPitch" /> is not 0,
        ...
        ///     <see cref="hostPointer" /> points to must be greater than or equal to <see cref="imageRowPitch" /> *
        ///     <see cref="imageHeight" />.
        ///     The size of each element in bytes must be a power of 2. The image data specified by <see cref="hostPointer" /> is
        ///     stored as a linear sequence of adjacent scanlines. Each scanline is stored as a linear sequence of image elements.
        /// </param>
        /// <param name="errorCode">
        ///     Will return an appropriate error code. If <see cref="errorCode" /> is <c>null</c>, no error
        ///     code is returned.
        /// </param>
        /// <returns>
        ///     Returns a valid non-zero image object and <see cref="errorCode" /> is set to <c>Result.Success</c> if the image
        ...
        ///     The size in bytes of each 2D slice in the 3D image. This must be 0 if <see cref="hostPointer" /> is <c>null</c> and
        ///     can be either 0 or greater than or equal to <see cref="imageRowPitch" /> * <see cref="imageHeight" /> if
        ///     <see cref="hostPointer" /> is not <c>null</c>. If <see cref="hostPointer" /> is not <c>null</c> and
        ///     <see cref="imageSlicePitch" /> equal to 0, <see cref="imageSlicePitch" /> is calculated as
        ///     <see cref="imageRowPitch" /> *
        ///     <see cref="imageHeight" />. If <see cref="imageSlicePitch" /> is not 0, it must be a multiple of the
        ///     <see cref="imageRowPitch" />.
        /// </param>
        /// <param name="hostPointer">
        ///     A pointer to the image data that may already be allocated by the application. The size of the buffer that
        ///     <see cref="hostPointer" /> points to must be greater than or equal to <see cref="imageSlicePitch" /> *
        ///     <see cref="imageDepth" />.
        ///     The size of each element in bytes must be a power of 2. The image data specified by <see cref="hostPointer" /> is
        ///     stored as a linear sequence of adjacent 2D slices. Each 2D slice is a linear sequence of adjacent scanlines. Each
        ///     scanline
